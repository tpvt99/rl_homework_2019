...
class MLPPolicy(BasePolicy):
    ...
    def __init__(self,
        ...
        **kwargs):
        ...
        # define forward pass
        if self.discrete:
            logits_na = BUILD_MLP(input_size=self.ob_dim, output_size=self.ac_dim, scope='discrete_logits', n_layers=self.n_layers, size=self.size)
            self.parameters = logits_na
        else:
            mean = BUILD_MLP(input_size=self.ob_dim, output_size=self.ac_dim, scope='continuous_logits', n_layers=self.n_layers, size=self.size)
    ...
    #TODO, removed by Phong
    # this function is done inside BUILD_MLP
    ...
    def action_sampling(self, inputs):
        if self.discrete:
            sample_ac = tf.squeeze(tf.multinomial(inputs, num_samples=1), axis=1)
        else:
            mean, logstd = inputs
            sample_ac = mean + tf.exp(logstd) *  tf.random_normal(tf.shape(mean), 0, 1)
        return sample_ac
    ...
    def define_log_prob(self, inputs):
        if self.discrete:
    ...
    def build_baseline_forward_pass(self):
        self.baseline_prediction = tf.squeeze(BUILD_MLP(input_size=self.ob_dim, output_size=1, scope='nn_baseline', n_layers=self.n_layers, size=self.size))

    ##################################

    def save(self, filepath):
        if self.discrete:
            self.parameters.save(filepath, format='tf')
        else:
            mean, logstd = self.parameters
            mean.save(filepath, format='tf')
            logstd.save(filepath, format='tf')

    def restore(self, filepath):
        self.parameters.load(filepath)
    ...
    def get_action(self, obs):
        ...
        return self.action_sampling(obs)
...
class MLPPolicyPG(MLPPolicy):
    ...
    def define_loss_function(self, obs, actions, adv_n):

        # define the log probability of seen actions/observations under the current policy
        if self.discrete:
            logprob_n = tf.distributions.Categorical(logits=obs).log_prob(actions)
        else:
            mean, logstd = self.parameters[0](obs), self.parameters[1]
            logprob_n = tfp.distributions.MultivariateNormalDiag(
                loc=mean, scale_diag=tf.exp(logstd)).log_prob(actions)
        ...
        loss = tf.reduce_sum(tf.math.multiply(logprob_n, adv_n))
