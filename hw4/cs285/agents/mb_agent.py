...
class MBAgent(BaseAgent):
    ...
    def train(self, ob_no, ac_na, re_n, next_ob_no, terminal_n):
        ...
        for i in range(self.ensemble_size):
            ...
            observations = ob_no[num_data_per_ens*i: num_data_per_ens*(i+1)]# TODO(Q1)
            actions = ac_na[num_data_per_ens*i: num_data_per_ens*(i+1)]# TODO(Q1)
            next_observations = next_ob_no[num_data_per_ens*i: num_data_per_ens*(i+1)]# TODO(Q1)

            # use datapoints to update one of the dyn_models
            model =  self.dyn_models[i]# TODO(Q1)
            log = model.update(observations, actions, next_observations,
                                self.actor.data_statistics)
